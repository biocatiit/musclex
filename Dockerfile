...
# RUN apt-get install -y python3.8 python3.8-dev python3-pip python3-pyqt5 gfortran
...
RUN echo "**** Installing Python ****" && \
    ...
    apt-get install -y build-essential python3.8 python3.8-distutils python3.8-dev python3-pip && \
    curl -O https://bootstrap.pypa.io/get-pip.py && \
    python3.8 get-pip.py && \
    rm -rf /var/lib/apt/lists/*

RUN pip3.8 install --upgrade pip
RUN pip3.8 install --upgrade distro
RUN pip3.8 install --upgrade pyopencl
RUN pip3.8 install --upgrade cython
RUN pip3.8 install --upgrade numpy
RUN pip3.8 install --upgrade opencv-python==4.1.2.30
RUN pip3.8 install --upgrade pyfai
RUN pip3.8 install --upgrade PyQt5
RUN pip3.8 install --upgrade musclexflibs
RUN pip3.8 install --upgrade fisx
RUN pip3.8 install --upgrade future

RUN pip3.8 install --upgrade musclex
#RUN pip3.8 install git+https://github.com/biocatiit/musclex.git@v1.15.7
