...
C Scalar arguments:
C
      REAL SMOO,TENS,PC1,PC2,XC,YC
      INTEGER IWID,JWID,ISEP,JSEP,DMIN,NPIX,NRAST
C
C Array arguments:
C
...
C========Mask out the backstop
C
      DO 20 J=1,NRAST
         DO 10 I=1,NPIX
            SQRAD=(I-IXC)**2+(J-IYC)**2
            M = (J-1)*NPIX + I
            B(M) = 0.
            IF(SQRAD.LE.DMIN**2)THEN
              BUF(M)=0.
            ENDIF
 10      CONTINUE
 20   CONTINUE
...
C
 ...
 40         CONTINUE
         ...
         ELSE
            DO 50 I=ISTRT,IEND
               M = (J-1)*NPIX + I 
               IF(BUF(M-NPIX).LT.0..OR.
     &            BUF(M+NPIX).LT.0.)KNOT(I) = .TRUE.
 50         CONTINUE
         ENDIF
C
