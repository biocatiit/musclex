...
class GMMParameterEditorDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.hullStartSpinBox.setToolTip(
            "Start position for convex hull background subtraction"
        )
        ...
        self.hullEndSpinBox.setToolTip(
            "End position for convex hull background subtraction"
        )
        ...
        self.refitBtn.setToolTip(
            "Re-fit parameters and save changes to fit_results.\nYou can continue editing after saving."
        )
    ...
    def populateParameters(self):
        ...
        if (
            self.working_hull_range
            and isinstance(self.working_hull_range, (tuple, list))
            and len(self.working_hull_range) >= 2
        ):
        ...
        while f"p_{i}" in fit_result:
            ...
            params_to_show[f"amplitude{i}"] = {
                ...
                "fixed": fit_result.get(
                    f"amplitude{i}_fixed", False
                ),  # Read fixed state
                "enabled": True,
        ...
        for param_name in sorted(params_to_show.keys(), key=_natural_sort_key):
            ...
            minSpin.setEnabled(
                bounds_enabled and pdict["enabled"] and not pdict["fixed"]
            )
    ...
    def _reset_bounds_for_row_by_tolerance(
        self, row: int, param_name: str, value: float
    ):
        """
        ...
        elif param_name == "common_sigma" or (
            param_name.startswith("sigma") and param_name != "common_sigma"
        ):
            ...
            if result:
                ...
                if box.hull_range:
                    ...
                    if hasattr(self.parent_tab, "snapshot_hull_range"):
                        self.parent_tab.snapshot_hull_range = copy.deepcopy(
                            box.hull_range
                        )
                ...
                if self.equalVarianceChkBx.isChecked():
                    ...
                    sigmas = [
                        result.get(f"sigma{i}", None)
                        for i in range(5)
                        if f"sigma{i}" in result
                    ]
                    sigma_info = (
                        f"Sigmas (first 5): {[f'{s:.2f}' for s in sigmas if s]}"
                    )
