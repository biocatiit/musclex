...
class AddIntensitiesMultipleExp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Pending experiment sources selected by the user (before loading)
        self._pending_sources: list = []

        # Custom select panel for AIME: add H5 files / folders as experiments
        ...
        _add_btn_row = QHBoxLayout()
        _add_h5_btn = QPushButton("+ Add H5 File(s)\u2026")
        _add_h5_btn.setMinimumHeight(32)
        _add_h5_btn.clicked.connect(self._on_add_h5_files)
        _add_folder_btn = QPushButton("+ Add Folder\u2026")
        _add_folder_btn.setMinimumHeight(32)
        _add_folder_btn.clicked.connect(self._on_add_folder)
        _add_btn_row.addWidget(_add_h5_btn)
        _add_btn_row.addWidget(_add_folder_btn)
        _sel_layout.addLayout(_add_btn_row)

        from PySide6.QtWidgets import QListWidget
        self._source_list_widget = QListWidget()
        self._source_list_widget.setMinimumHeight(120)
        self._source_list_widget.setSelectionMode(QListWidget.ExtendedSelection)
        self._source_list_widget.setToolTip("Selected experiments (H5 files or folders)")
        _sel_layout.addWidget(self._source_list_widget)

        _action_row = QHBoxLayout()
        _remove_btn = QPushButton("Remove Selected")
        _remove_btn.clicked.connect(self._on_remove_selected_source)
        _clear_btn = QPushButton("Clear All")
        _clear_btn.clicked.connect(self._on_clear_sources)
        ...
        _action_row.addWidget(_remove_btn)
        _action_row.addWidget(_clear_btn)
    ...
    def _on_add_h5_files(self):
        """Open a file dialog to select one or more H5 files as experiments."""
        from PySide6.QtWidgets import QFileDialog
        files, _ = QFileDialog.getOpenFileNames(
            self, "Select H5 Experiment File(s)", "",
            "HDF5 Files (*.h5 *.hdf5)"
        )
        added = 0
        for f in files:
            if f not in self._pending_sources:
                self._pending_sources.append(f)
                added += 1
        if added:
            self._refresh_source_list()

    def _on_add_folder(self):
        """Open a folder dialog to select a directory as one experiment."""
        from PySide6.QtWidgets import QFileDialog
        folder = QFileDialog.getExistingDirectory(
            self, "Select Experiment Folder", "",
            QFileDialog.ShowDirsOnly | QFileDialog.DontResolveSymlinks,
        )
        if folder and folder not in self._pending_sources:
            self._pending_sources.append(folder)
            self._refresh_source_list()

    def _on_remove_selected_source(self):
        """Remove highlighted rows from the pending source list."""
        rows = sorted(
            {self._source_list_widget.row(item)
             for item in self._source_list_widget.selectedItems()},
            reverse=True,
        )
        for row in rows:
    ...
    def _refresh_source_list(self):
        """Sync the list widget to self._pending_sources."""
        self._source_list_widget.clear()
        for src in self._pending_sources:
            item_text = os.path.basename(src.rstrip('/\\'))
            from PySide6.QtWidgets import QListWidgetItem
            item = QListWidgetItem(item_text)
            item.setToolTip(src)
            self._source_list_widget.addItem(item)
        n = len(self._pending_sources)
        self._load_sources_btn.setEnabled(n > 0)
        self._exp_dirs_label.setText(
            f"{n} experiment(s) pending" if n else ""
        )

    def _on_load_sources(self):
        """Load all pending sources into FileManager and build the table."""
        if not self._pending_sources:
            return

        fm = self.workspace.navigator.file_manager
        fm.load_from_sources(self._pending_sources)

        # Use common parent of all sources as the output/settings root
        parent_dirs = [
            os.path.dirname(s) if os.path.isfile(s) else s
            for s in self._pending_sources
        ]
        try:
            parent_dir = os.path.commonpath(parent_dirs)
        except ValueError:
            parent_dir = parent_dirs[0]

        self._on_directories_loaded(parent_dir, list(self._pending_sources))
