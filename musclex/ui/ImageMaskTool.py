...
def displayImage(imageArray, minInt, maxInt, rot=0):
    ...
    flippedImageArray = rotate(
        np.flipud(imageArray), rot, reshape=False
    )  # flip the image vertically to match what is displayed on the main GUI
    ...
    if np.max(flippedImageArray) == np.min(flippedImageArray):
        ...
        normFlippedImageArray = normFlippedImageArray.astype(
            np.uint8
        )  # Convert to 8-bit
...
def displayImageWithMasks(
    imageArray, minInt, maxInt, lowMask, highMask, drawnMask, rMask=None, rot=0
):
    """
...
class ImageMaskerWindow(QDialog):
    def __init__(
        ...
        self.lowThreshMaskData = (
            None  # Attribute to store the mask created by the lower bound threshold
        )
        self.dilatedLowThreshMaskData = None  # Attribute to store the mask created by the lower bound threshold after dilation
        self.highThreshMaskData = (
            None  # Attribute to store the mask created by the upper bound threshold
        )
    ...
    def initUI(self):
        ...
        self.showBlankImageChkbx.setToolTip(
            "Display the loaded empty-cell image instead of the data image"
        )
        ...
        self.rminSpinBox.setMaximum(
            math.ceil(math.sqrt(self.orig_size[0] ** 2 * self.orig_size[1] ** 2))
        )
        ...
        self.clampNegativeValuesChkbx.setToolTip(
            "Sets all negative values after subtraction to 0"
        )
    ...
    def showBlankImage(self):
        if self.showBlankImageChkbx.isChecked():
            ...
            if self.maskedImage is not None:
                scaledPixmap = displayImage(
                    self.maskedImage.data, self.minInt, self.maxInt, self.rot_angle
                )
    ...
    def enableLowMaskThres(self):
        if self.maskLowThresChkbx.isChecked():
            ...
            if (
                self.highThreshMaskData is not None
    ...
    def enableHighMaskThres(self):
        if self.maskHighThreshChkbx.isChecked():
            ...
            if (
                self.lowThreshMaskData is not None
                and self.maskLowThresChkbx.isChecked()
            ):
    ...
    def computeThreshMaskData(self):
        ...
        if self.maskLowThresChkbx.isChecked():
            if self.lowMaskDilationChkbx.isChecked():
                self.lowThreshMaskData = np.where(
                    originalImageArray <= self.maskLowThreshVal, 0, 1
                ).astype(np.uint8)
        ...
        if self.maskHighThreshChkbx.isChecked():
            if self.highMaskDilationChkbx.isChecked():
                self.highThreshMaskData = np.where(
                    originalImageArray >= self.maskHighThreshVal, 0, 1
                ).astype(np.uint8)
    ...
    def enableSubtractSlider(self):
        ...
        if self.subtractBlankChkbx.isChecked():
            ...
            if self.maskedImage is not None:
                ...
                scaledPixmap = displayImage(
                    self.maskedImage, self.minInt, self.maxInt, self.rot_angle
                )
                self.imageLabel.setPixmap(scaledPixmap)
            else:
                min_value = np.min(self.imageData)
                scaledPixmap = displayImage(
                    self.imageData, self.minInt, self.maxInt, self.rot_angle
                )
                self.imageLabel.setPixmap(scaledPixmap)
            if min_value < 0:
                self.negativeValuesLabel.setText(
                    "Negative Values in Image (Min: {}) ".format(min_value)
                )
    ...
    def refreshMask(self):
        if self.showMaskComboBox.currentText() == "Show Mask Only":
            ...
            self.computeCombinedMask()
            self.applyMask()
            scaledPixMap = displayImageWithMasks(
                self.imageData,
                self.minInt,
                self.maxInt,
                self.dilatedLowThreshMaskData,
                self.dilatedHighThreshMaskData,
                self.drawnMaskData,
                self.r_mask_data,
                rot=self.rot_angle,
            )
        ...
        if self.showBlankImageChkbx.isChecked():
            ...
            if self.isHDF5:
                ...
                tif_img = fabio.pilatusimage.pilatusimage(
                    data=data, header=fabio_img.getheader()
                )
    ...
    def applyMask(self):
        ...
        if self.dir_path:
            ...
            scaledPixmap = displayImage(
                maskedImageArray, self.minInt, self.maxInt, self.rot_angle
            )
            self.imageLabel.setPixmap(scaledPixmap)
    ...
    def okClicked(self):
        if (
            self.maskData is None
            and self.subtractBlankChkbx.isChecked() == False
            and self.computedMaskData is None
            and self.drawnMaskData is None
        ):
            errMsg = QMessageBox()
            errMsg.setText(
                "No mask data or blank image was provided. Please draw a mask or select a blank image."
            )
            ...
            if self.maskData is not None:
                ...
                rot_mat = cv2.getRotationMatrix2D(
                    (width / 2, height / 2), -self.rot_angle, 1.0
                )

                rotated_mask = cv2.warpAffine(
                    self.maskData.astype(np.uint8), rot_mat, (width, height)
                )
                ...
                if self.trans_mat is not None:
                    ...
                    self.trans_mat = np.float32(
                        [[1, 0, 0], [0, 1, 0]]
                    )  # transformation by 0,0 i.e. no transformation
                    inv_trans_mat = self.trans_mat.copy()
                ...
                try:
                    cropped_mask = translated_mask[
                        0 : self.orig_size[0], 0 : self.orig_size[1]
                    ]
                    fabio.tifimage.tifimage(data=cropped_mask).write(
                        join(path, "mask.tif")
                    )
    ...
    def subtractBlankImage(self):
        ...
        scaledPixmap = displayImage(
            self.subtractedImage, self.minInt, self.maxInt, self.rot_angle
        )
        self.imageLabel.setPixmap(scaledPixmap)
    ...
    def computeRadialMasks(self):
        ...
        low = (
            np.ones_like(r_mask)
            if self.dilatedLowThreshMaskData is None
            else self.dilatedLowThreshMaskData
        )
        high = (
            np.ones_like(r_mask)
            if self.dilatedHighThreshMaskData is None
            else self.dilatedHighThreshMaskData
        )
