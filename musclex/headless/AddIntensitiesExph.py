    ...
    def loadImages(self):
        if self.mode == "aise":
            if self.type == "folder":
                ...
                with fabio.open(self.path) as series:
                    for frame in series.frames():
                        self.image_files.append(
                            ifHdfReadConvertless(self.path, frame.data).astype(
                                np.float32
                            )
                        )
                ...
                self.file_name_groups = [
                    self.file_names[i : i + self.nbOfFrames]
                    for i in range(0, len(self.file_names), self.nbOfFrames)
                ]
            ...
            self.image_groups = [
                self.image_files[i : i + self.nbOfFrames]
                for i in range(0, len(self.image_files), self.nbOfFrames)
            ]
                        ...
                        if self.isHdf5:
                            ...
                            with fabio.open(f) as series:
                                for frame in series.frames():
                                    ...
                                    self.numberToFilesMap[frame.index].append(
                                        temp_filename
                                    )
                                    self.numberToFilesMapPath[frame.index].append(
                                        ifHdfReadConvertless(
                                            self.path, frame.data
                                        ).astype(np.float32)
                                    )
                        else:
                        ...
                        if os.path.isdir(temp_path):
                            ...
                            for fname2 in os.listdir(temp_path):
                                if isImg(fname2):
                                    ...
                                    self.numberToFilesMapPath[i].append(
                                        os.path.join(temp_path, fname2)
                                    )
                                i += 1
    ...
    def startProcessing(self):
        ...
        if self.mode == "aise":
            ...
            with multiprocessing.Pool(
                initializer=init, initargs=(l,), processes=multiprocessing.cpu_count()
            ) as pool:
                pool.map(
                    addImageChunk,
                    [
                ...
                with multiprocessing.Pool(
                    ...
                    pool.map(
                        ...
                        [
                            ...
                            for (i, group) in self.numberToFilesMapPath.items()
                        ],
                    )
            else:
                with multiprocessing.Pool(
    ...
    if len(chunk) > 0:
        ...
        if settings["compress"] == True:
            if "maskPath" in settings:
                ...
                mask_tif_img.save(
                    os.path.join(output, masked_filename), compression="tiff_lzw"
                )
            ...
            if "maskPath" in settings:
                fabio.tifimage.tifimage(sum_img).write(
                    os.path.join(output, masked_filename)
                )
...
def generateSumIng(chunk, settings, path, filenames, mode):
    ...
    if settings["avgInsteadOfSum"] == True:
        ...
        if "detector" in settings:
            sum_img = averageImages(
                images, preprocessed=True, man_det=settings["detector"]
            )
        else:
        ...
        for img_path in chunk:
            ...
            if not isinstance(sum_img, int) and (
                img.shape[0] > sum_img.shape[0] or img.shape[1] > sum_img.shape[1]
            ):
