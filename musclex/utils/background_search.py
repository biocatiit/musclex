...
def get_projection(
    img, gap, orientation=0, avg=False, half=False, offset=0, center=None
):
    """
...
def find_fwhm(histogram, peak_index=None, rel_height=0.5):
    ...
    try:
        widths, _, _, _ = peak_widths(hist_array, [peak_index], rel_height=rel_height)
...
def create_peak_mask(image_shape, peak_positions, peak_width=5, center=None):
    ...
    for pos in peak_positions:
        mask[
            h_center - peak_width // 2 : h_center + peak_width // 2,
            v_center - pos - peak_width // 2 : v_center - pos + peak_width // 2,
        ] = False
        mask[
            h_center - peak_width // 2 : h_center + peak_width // 2,
            v_center + pos - peak_width // 2 : v_center + pos + peak_width // 2,
...
def create_rectangle_mask(height, width, x_length=None, y_height=None):
    ...
    mask[
        center_y - y_height // 2 : center_y + y_height // 2,
        center_x - x_length // 2 : center_x + x_length // 2,
    ] = 0
    return mask.astype(int)
...
class GaussianArtificialData(ArtificialData):
    ...
    def pre_compute_kernel(self):
        self.kernel_width = int(
            6 * self.sigma_x
        )  # 6 sigma should cover ~99.7% of distribution
        ...
        self.gaussian_kernel = self.amplitude * np.exp(
            -(KX**2 / (2 * self.sigma_x**2) + KY**2 / (2 * self.sigma_y**2))
        )
    ...
    def create_image(self, point_x, point_y):
        ...
        self.image[y_start:y_end, x_start:x_end] += self.gaussian_kernel[
...
def prepare_synthetic_eval_pair(
    syn_fold_syn, syn_fold_base, synthetic_data, synthetic_mask, gen_mask=None
):
    """
...
def full_eval_metrics(
    ...
    if syn_str is None or syn_mask is None:
        ...
        mse = artificial_data_offset_mse(
            syn_img, syn_str, syn_mask, gen_mask_fold, normalize=True
        )
    ...
    if normalize_metrics:
        ...
        share_non_baseline_pixels = share_non_baseline_pixels / _coerce_fraction_mean(
            mean_values["SHARE_NON_BASELINE_MEAN"],
            MEAN_METRIC_VALUES["SHARE_NON_BASELINE_MEAN"],
        )
...
def artificial_data_mse(
    dimg, syn_str, syn_mask, gen_mask=None, normalize=True, eps=1e-12
):
    """
...
def artificial_data_offset_mse(
    dimg, syn_str, syn_mask, gen_mask=None, normalize=True, eps=1e-12
):
    """
...
def evaluate_loss(
    ...
    if return_details:
        return {
            ...
            "metric_weights": (
                metric_weights if metric_weights is not None else dict(WEIGHTS)
            ),
        }
...
def compute_loss_from_raw(raw_metrics, mean_metric_values=None, metric_weights=None):
    ...
    share_neg_synthetic = raw_metrics.get(
        "Share_Neg_Synthetic", 0.0
    ) / _coerce_fraction_mean(
        mean_values["SHARE_NEG_SYN_MEAN"], MEAN_METRIC_VALUES["SHARE_NEG_SYN_MEAN"]
    )
    share_non_baseline = raw_metrics.get(
        "Share_Non_Baseline", 0.0
    ) / _coerce_fraction_mean(
        mean_values["SHARE_NON_BASELINE_MEAN"],
        MEAN_METRIC_VALUES["SHARE_NON_BASELINE_MEAN"],
    )
    share_neg_connected = raw_metrics.get(
        "Share_Neg_Connected", 0.0
    ) / _coerce_fraction_mean(
        mean_values["SHARE_NEG_CON_MEAN"], MEAN_METRIC_VALUES["SHARE_NEG_CON_MEAN"]
    )
...
def _build_result_record(
    method, params, raw_metrics, loss, cache_key, from_cache, mean_metric_values=None
):
    normalized_metrics = {}
    if isinstance(raw_metrics, dict):
        ...
        normalized_metrics = {
            "MSE": raw_metrics.get("MSE", 0.0) / mean_values["MSE_SYN_MEAN"],
            "Share_Neg_Synthetic": raw_metrics.get("Share_Neg_Synthetic", 0.0)
            / _coerce_fraction_mean(
                mean_values["SHARE_NEG_SYN_MEAN"],
                MEAN_METRIC_VALUES["SHARE_NEG_SYN_MEAN"],
            ),
            "Share_Non_Baseline": raw_metrics.get("Share_Non_Baseline", 0.0)
            / _coerce_fraction_mean(
                mean_values["SHARE_NON_BASELINE_MEAN"],
                MEAN_METRIC_VALUES["SHARE_NON_BASELINE_MEAN"],
            ),
            "Share_Neg_Connected": raw_metrics.get("Share_Neg_Connected", 0.0)
            / _coerce_fraction_mean(
                mean_values["SHARE_NEG_CON_MEAN"],
                MEAN_METRIC_VALUES["SHARE_NEG_CON_MEAN"],
            ),
            "Smoothness": raw_metrics.get("Smoothness", 0.0)
            / mean_values["SMOOTH_MEAN"],
        }
...
def apply2DConvexhull(img, rmin, step=1):
    ...
    pchiplines2 = convolve1d(pchiplines, [1, 2, 1], axis=0) / 4.0
    ...
    background = qfu.make2DConvexhullBG2(
        pchiplines3, img.shape[1], img.shape[0], center[0], center[1], rmin, rmax, step
    )
...
def applyWhiteTophat(img, radius):
    ...
    tophat = cv2.morphologyEx(
        img32, cv2.MORPH_TOPHAT, kernel, borderType=cv2.BORDER_REPLICATE
    )
    return img - tophat


def applySmoothedBGSub(
    fold, fwhm=15, boxcar_x=15, boxcar_y=15, cycles=250, typ="gauss"
):
    """
...
def applyCircularlySymBGSub2(
    fold, rmin, radial_bin=10, smooth=5, pc1=0, pc2=25, tension=1
):
    """
...
def applyBackgroundRemoval(
    ...
    if method == "None":
        ...
        bg = applyCircularlySymBGSub2(
            tmp_avg_fold,
            tmp_rmin,
            radial_bin=params["radial_bin"],
        ...
        bg = applyRovingWindowBGSub(
            tmp_avg_fold,
            tmp_center,
            win_size_x=params["win_size_x"],
            win_size_y=params["win_size_y"],
            win_sep_x=params["win_sep_x"],
            win_sep_y=params["win_sep_y"],
            smooth=params["smooth"],
            tension=params["tension"],
            pc1=params["cirmin"],
            pc2=params["cirmax"],
        ...
        bg = applySmoothedBGSub(
            fold=tmp_avg_fold, fwhm=params["fwhm"], cycles=params["cycles"], typ="gauss"
        )
    elif method == "Smoothed-BoxCar":
        bg = applySmoothedBGSub(
            fold=tmp_avg_fold,
            boxcar_x=params["boxcar_x"],
            boxcar_y=params["boxcar_y"],
            cycles=params["cycles"],
            typ="boxcar",
        )
    else:
        print(
            f"Unknown background removal method: {method}. Not applying background removal."
        )
        bg = np.zeros_like(avg_fold)
...
def optimize(method, **kwargs):
    ...
    from musclex.utils.qf_defaults import (
        DEFAULT_OPTIMIZATION_STEPS,
        parse_optimization_steps,
        DEFAULT_EARLY_STOP,
    ...
    def evaluate_candidate(test_params, use_timeout=True):
        ...
        cache_key = _build_raw_metrics_cache_key(
            method, test_params, params_dict, kwargs
        )
        ...
        if use_timeout:
            loss, result = process_file_with_timeout(
                test_params, method=method, **kwargs
            )
        else:
    ...
    for param_idx in param_order:
        ...
        while step_idx < len(steps):
            ...
            candidates = [
                max(bounds[param_idx][0], min(bounds[param_idx][1], v))
                for v in candidates
            ]
