...
from .bg_fitting.cone_model import ConeModelConfig, model_eval, fit_cone_background
...
# --------------------------------------------------------------------------- #
def run_step0_projection_bg(img, mask, rmin, rmax, cfg=None,
                            save_params=False, save_img=False, save_dir=None,
                            name="image"):
    ...
    from .background_search import makeFullImage
    ...
    for xs, ys in zip(px, py):
        ...
        if m.sum() < 30:
            ...
            continue
        x_fit = xs[m]; y_fit = ys[m]
        x_max = float(np.max(x_fit)) or 1.0
        y_max = float(np.max(y_fit)) or 1.0
        try:
            with maybe_quiet(cfg.quiet):
                fr = fit_cone_background(
                    (x_fit / x_max).reshape(-1, 1), y_fit / y_max,
                    cfg=cone_cfg, transform_anscombe=False,
                    loss="huber", asymmetric_weight=2.0)
            y_pred = model_eval(xs / x_max, fr.coeffs, fr.qmin, fr.qmax,
                                fr.config) * y_max
    ...
    dbg = qfu.make2DConvexhullBG2(
        proj_pred, quad.shape[1], quad.shape[0], center[0], center[1],
        rmin, rmax, step)
    ...
    if (save_params or save_img) and save_dir:
        ...
        if save_params:
            np.savez(os.path.join(save_dir, f"{name}_step0_params.npz"),
                     projections=proj_pred, rmin=rmin, rmax=rmax, step=step)
...
def fit_equator(target, mask, cfg, quiet=True):
    ...
    mask_ds = _downsample(mstrip.astype(float), ds, aa=False) > 0.5 if ds > 1 else mstrip

    with maybe_quiet(quiet):
        fitter = EllipticalSAXSFitter(pattern_ds, mask=mask_ds, model="elliptical",
                                      n_peaks=1, downsample_factor=1)
        result = fitter.fit(n_peaks=1, loss="linear", neg_weight=cfg.eq_neg_weight,
                            pos_weight=cfg.pos_weight, max_nfev=cfg.eq_max_nfev,
                            model="elliptical")
        params = fitter.fitted_params.copy()
        equator_full = bfu.reconstruct_equator(params, target.shape, ds, max_norm,
                                               cfg.equator_keep_baseline)
        equator_model = bfu.reconstruct_equator(params, target.shape, ds, max_norm,
                                                keep_baseline=True)
    eq_metrics = bfu.compute_metrics(strip, equator_model[sl, :], mstrip, len(params))
    info = {"success": bool(getattr(result, "success", False)),
...
def fit_general(target, mask, cfg, comp2, quiet=True):
    ...
    r, prof = bfu.wedge_radial_profile(pattern_ds, mask_ds, x0, y0,
                                       cfg.wedge_min, cfg.wedge_max)
    init = bfu.init_from_wedge(r, prof, max(cfg.rmin_mask // ds, 1)) if r is not None else None

    with maybe_quiet(quiet):
        fitter = EllipticalSAXSFitter(pattern_ds, mask=mask_ds,
                                      model="circular_exponential", K=0, n_peaks=1,
                                      downsample_factor=1, comp1="exponential",
                                      comp2=comp2, comp3=None, aspect=True)
        base = fitter.get_initial_params(n_peaks=1, K=0,
                                         model="circular_exponential", verbose=False)
        if init is not None:
        ...
        result = fitter.fit(initial_params=base, n_peaks=1, K=0, max_nfev=cfg.gen_max_nfev,
                            model="circular_exponential", comp1="exponential",
                            comp2=comp2, comp3=None, aspect=True, **kw)
        params = fitter.fitted_params.copy()
...
def fit_general_best(target, mask, cfg, quiet=True, comp2=None):
    ...
    h = target.shape[0]; hc = h // 2
    hw = min(cfg.gen_hwidth, h); hw -= hw % 2
    sl = slice(hc - hw // 2, hc + hw // 2)
    cands = []
    for comp2 in candidates:
        try:
            general, params, norm, wedge, cost = fit_general(target, mask, cfg, comp2, quiet)
        except Exception as e:  # noqa: BLE001
            print(f"    [comp2={comp2} FAIL] {e}")
            continue
        metrics = bfu.compute_metrics(target[sl, :], general[sl, :], mask[sl, :], len(params))
        score = bfu.decision_score(metrics)
        cands.append({"comp2": comp2, "general": general, "params": params,
                      "norm": norm, "score": score, "cost": cost,
                      "metrics": metrics, "wedge": wedge})
    if not cands:
...
def two_stage_iterative_fit(img, general_mask, equator_mask=None, rmin=30, rmax=None,
                            cfg=None, progress_cb=None, rminrmax_mask=None):
    ...
    for it in range(1, cfg.iters + 1):
        ...
        general_red = bfu.reconstruct_general(gen_params_red, img_fit.shape,
                                              gen_best["comp2"], cfg.downsample_factor,
                                              gen_best["norm"])
        ...
        iter_records.append({
            "round": it, "equator": equator, "general": general,
            "eq_params": eq_params, "gen_params": gen_best["params"],
            "gen_norm": gen_best["norm"], "eq_norm": eq_norm,
            "comp2": gen_best["comp2"], "gen_cost": gen_best["cost"],
            "gen_score": gen_best["score"], "gen_metrics": gen_best["metrics"],
            "eq_cost": eq_info["cost"], "eq_metrics": eq_info["metrics"],
            "neg": neg, "neg_red": neg_red, "lobe_ok": lobe_ok, "lobe": lobe_info,
        })
    ...
    if cfg.auto_reduce:
        ...
        eq_red_auto, _ = bfu.auto_reduce(
            eq_frac, cfg.reduction_max_equator, cfg.reduction_step_equator,
    ...
    if cfg.auto_reduce:
        def bl_frac(r):
            ...
            return bfu.negative_stats(base - eq_fit_final - _gen_fit(total), valid)["frac_negative"]
        bl_red_auto, _ = bfu.auto_reduce(
            bl_frac, cfg.reduction_max_baseline, cfg.reduction_step_baseline,
