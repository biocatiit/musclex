...
coefficients = {"image": 0.33, "center": 0.33, "angle": 0.33}
...
def detectImages(folder_path, max_intensity, distance_mode, coefficients):
    ...
    center_medians = np.median(
        [np.linalg.norm(np.array(item.center)) for item in items]
    )
...
def find_inconsistencies(items, center_median, angle_median):
    ...
    distances = [
        measure_distance(items[i], items[i + 1], center_median, angle_median)
        for i in range(len(items) - 1)
    ]
    distances.append(
        measure_distance(items[-1], items[0], center_median, angle_median)
    )  # compare last image and first image
    ...
    inconsistent_items_indices = [
        i for i in range(len(distances)) if distances[i] > threshold
    ]
