...
class EllipticalSAXSFitter:
    ...
    def __init__(self, pattern, crop=[None, None], center=None, mask=None, model='elliptical', n_peaks=1, downsample_factor=1, K=1, kernel='exponential', comp1=None, comp2=None, comp3=None, aspect=False):
        """
        ...
        if downsample_factor > 1:
            ...
            pattern = resize(pattern, 
                             (pattern.shape[0] // downsample_factor, pattern.shape[1] // downsample_factor), 
                             anti_aliasing=True)
            if mask is not None:
                mask = resize(mask, 
                              (mask.shape[0] // downsample_factor, mask.shape[1] // downsample_factor), 
    ...
    def elliptical_intensity(self, params, return_components=False):
        ...
        for i in range(n_peaks):
            ...
            g_v = np.zeros_like(v)
            v_positions = [v0 % 360.0, (180.0 - v0) % 360.0, (180.0 + v0) % 360.0, (360.0 - v0) % 360.0]
    ...
    _BASE_RADIAL = ('exponential', 'gaussian', 'stretched', 'lorentzian', 'lorentzian2', 'powerlaw')
    ...
    def _resolve_components(self):
        ...
        if c1 is not None:
            ...
            for name, c in (('comp2', c2), ('comp3', c3)):
                if c is not None and c not in self._BASE_RADIAL and c not in self._COMP2_SPECIAL_NPARAM:
    ...
    def _build_comp2(self, kernel, extras, R, R_edge):
        ...
        if kernel in self._BASE_RADIAL:
            ...
            beta2 = max(extras[2], 1e-3) if (kernel == 'stretched' and len(extras) >= 3) else 1.0
            n2 = max(extras[2], 1e-3) if (kernel == 'powerlaw' and len(extras) >= 3) else 4.0
    ...
    def anisotropic_circular_intensity(self, params, order, return_components=False):
        ...
        if use_aspect:
            ...
            e = 1.0
        a0_amp   = params[idx]; idx += 1
        a0_scl   = params[idx]; idx += 1
        ...
        for k in range(order):
            cos_amp.append(params[idx]);     idx += 1
            sin_amp.append(params[idx]);     idx += 1
            cos_scl.append(params[idx]);     idx += 1
            sin_scl.append(params[idx]);     idx += 1
        ...
        idx += n1_extra
        beta1 = c1_extra[0] if (comp1_kernel == 'stretched' and len(c1_extra) >= 1) else 1.0
        n_pl1 = c1_extra[0] if (comp1_kernel == 'powerlaw' and len(c1_extra) >= 1) else 4.0
    ...
    def compute_intensity(self, params, return_components=False):
        ...
        if model == 'elliptical':
            ...
            return self.anisotropic_circular_intensity(params, order, return_components=return_components)
            ...
            if return_components:
                I_ell, comps_ell = self.elliptical_intensity(params_ell, return_components=True)
                I_circ, comps_circ = self.anisotropic_circular_intensity(params_circ, order, return_components=True)
                return I_ell + I_circ, comps_ell + comps_circ
            else:
                I_ell = self.elliptical_intensity(params_ell, return_components=False)
                I_circ = self.anisotropic_circular_intensity(params_circ, order, return_components=False)
                return I_ell + I_circ
        else:
    ...
    def fit(self, initial_params=None, n_peaks=1, K=0, neg_weight=1.0, pos_weight=1.0,
            max_nfev=200, loss='linear', model='elliptical', kernel='exponential',
            comp1=None, comp2=None, comp3=None, log_space=False, aspect=None):
        """
        ...
        for i in range(len(initial_params)):
            print(f"Parameter {i}: initial = {initial_params[i]:.3f}, bounds = [{bounds[0][i]:.3f}, {bounds[1][i]:.3f}]")
            if initial_params[i] < bounds[0][i] or initial_params[i] > bounds[1][i]:
                print(f"Warning: initial parameter {i} = {initial_params[i]:.3f} out of bounds [{bounds[0][i]:.3f}, {bounds[1][i]:.3f}]")
        ...
        if self.downsample_factor > 1:
            self.fitted_params = self._scale_params_to_original_resolution(self.fitted_params)
    ...
    def get_initial_params(self, n_peaks=1, K=1, model='elliptical', verbose=False):
        ...
        if model == 'elliptical':
            ...
            u_tmp, _ = self.cartesian_to_elliptic_np(self.X_centered, self.Y_centered, A0, degrees=True, normalize=True)
            ...
            d_raw = np.log(np.expm1(d0)) if d0 > 1 else d0  # raw param s.t. softplus(d_raw)=d0
            ...
            for _ in range(K):
                params.extend([0.0, 0.0, 0.0, 0.0])  # a_kc_amp, a_ks_amp, a_kc_dec, a_ks_dec
            ...
            params.extend(self._default_comp_params(comp2_kernel, amp0, d0, r_max, d_raw_hi))
            params.extend(self._default_comp_params(comp3_kernel, amp0, d0, r_max, d_raw_hi))

            if verbose:
                print(f"Initial parameters (circular comp1={comp1_kernel}, "
                      f"comp2={comp2_kernel}, comp3={comp3_kernel}, K={K}): {params}")
            return np.array(params)

        elif model == 'elliptical+circular_exponential':
            params_ell = list(self.get_initial_params(n_peaks=n_peaks, K=K, model='elliptical', verbose=False))
            params_circ = list(self.get_initial_params(n_peaks=n_peaks, K=K, model='circular_exponential', verbose=False))
            params = params_ell + params_circ
            if verbose:
                print(f"Initial parameters (elliptical+circular_exponential, {n_peaks} peak(s), K={K}): {params}")
            return np.array(params)
    ...
    def get_parameter_bounds(self, n_peaks=1, K=1, model='elliptical'):
        ...
        if model == 'elliptical':
            ...
            upper = [np.inf, 35.0]
            if getattr(self, 'aspect', False):
                lower += [0.2];   upper += [5.0]      # e: aspect ratio (~oval)
            ...
            if comp1_kernel == 'stretched':
                lower += [0.1];   upper += [3.0]      # beta
            elif comp1_kernel == 'powerlaw':
                lower += [0.5];   upper += [8.0]      # n
            ...
            lower_ell, upper_ell = self.get_parameter_bounds(n_peaks=n_peaks, K=K, model='elliptical')
            lower_circ, upper_circ = self.get_parameter_bounds(n_peaks=n_peaks, K=K, model='circular_exponential')
    ...
    def plot_results(self, figsize=(15, 5)):
        ...
        # Original
        im0 = axes[0].imshow(self.pattern, origin='lower', 
        ...
        # Fitted
        im1 = axes[1].imshow(fitted, origin='lower',
                             vmin=vmin, vmax=vmax, cmap='viridis')
        ...
        im2 = axes[2].imshow(residual, origin='lower',
    ...
    def plot_components(self, figsize=None):
        ...
        fitted, components = self.compute_intensity(self.fitted_params, return_components=True)
        ...
        titles = ['Equatorial Streak', 'Lamellar Reflection', 'Fibrillar Interference', 'Circular Exp.']
    ...
    def print_parameters(self, params=None):
        ...
        if model == 'elliptical':
            ...
            print()
            n_peaks = (len(params) - 2) // 6
            peak_names = ['Equatorial Streak', 'Lamellar Reflection', 'Fibrillar Interference']
            for i in range(n_peaks):
                idx = 2 + i * 6
                print(f"Peak {i+1}: {peak_names[i] if i < len(peak_names) else 'Unknown'}")
        ...
        if model == 'circular_exponential':
            ...
            rest = params[idx + n1:]
            self._print_comp_params(comp1_kernel, [params[2], params[3]] + list(c1_extra), label='Comp1')
        ...
        if model == 'elliptical+circular_exponential':
            ...
            scale_name = 'xi' if kernel in ('lorentzian', 'lorentzian2') else ('c' if kernel == 'powerlaw' else 'D')
    ...
    def plot_coordinates(self, figsize=(15, 5), A=100, degrees=True, normalize=True, polar=False):
        ...
        if polar:
            ...
            u, v = self.cartesian_to_elliptic_np(
                self.X_centered, self.Y_centered, A, degrees=degrees, normalize=normalize
            )
    ...
    def plot_elliptical_coordinate_system(self, A=100, figsize=(12, 5), clip=[0, 500]):
        ...
        valid_mask = np.isfinite(pattern_flat) & np.isfinite(u_flat) & np.isfinite(v_flat)
        ...
        im0 = axes[0].imshow(np.clip(self.pattern, clip[0], clip[1]), origin='lower', cmap='viridis'
        ...
        im1 = axes[1].imshow(np.clip(pattern_elliptical, clip[0], clip[1]), origin='lower',
                            cmap='viridis',
    ...
    def plot_polar_coordinate_system(self, figsize=(6, 5), clip=[0, 500]):
        ...
        # Remove invalid points
        valid_mask = np.isfinite(pattern_flat) & np.isfinite(r_flat) & np.isfinite(theta_flat)
        ...
        im = ax.imshow(np.clip(pattern_polar, clip[0], clip[1]), origin='lower',
                        cmap='viridis',
    ...
    def plot_radial_azimuthal_profiles(self, A=100, figsize=(15, 5)):
        ...
        H, u_edges, v_edges = np.histogram2d(
            u_flat[valid_mask], v_flat[valid_mask], 
            bins=[50, 180], 
        ...
        im = axes[2].imshow(H_avg.T, origin='lower', cmap='viridis',
...
if __name__ == "__main__":
    ...
    streak_y_width = 30
    streak_x_width = 100
    streak = streak_intensity * np.exp(-((Y - y_center)**2) / (2 * streak_y_width**2)) * \
