...
def initial_params(
    ...
    if cfg.use_upturn:
        params += [
            max(0.0, 0.1 * np.nanmax(y)),
            max(1e-3, cfg.q0_init),
            np.clip(cfg.p_init, 0.5, 4.0),
        ]
...
def fit_cone_background(
    ...
    def residuals(p):
        ...
        weighted_residuals = np.where(
            raw_residuals < 0, asymmetric_weight * raw_residuals, raw_residuals
        )
